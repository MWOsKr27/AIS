...
BEGIN
	...
	BEGIN
		...
		SET @batch = 'C:\AIS\Batch\Implementation\write_DSN_parameters.bat ' + @param
		EXEC master..xp_CMDShell @batch

		SET @param = @project_name
		SET @param += ' ' + CAST((SELECT item_account FROM lu_request WHERE request_id = @request_id) AS VARCHAR)
		SET @param += ' ' + CAST((SELECT project_id FROM lu_request WHERE request_id = @request_id) AS VARCHAR)
		SET @batch = 'C:\AIS\Batch\Implementation\write_duplication_parameters.bat ' + @param
		EXEC master..xp_CMDShell @batch
	...
	BEGIN
		SET @param = (SELECT p.project_name FROM lu_project p JOIN lu_request r ON p.project_id = r.project_id WHERE r.request_id = @request_id)
		SET @param += ' ' + (SELECT retailer_string FROM lu_request WHERE request_id = @request_id)
		SET @param += ' ' + CAST((SELECT flag FROM lu_request WHERE request_id = @request_id) AS VARCHAR)
